...
public class BaseTranslator {
    ...
    public static int[] convertBase(int[] digits, int baseA,
        ...
        int[] digitsNew = new int[digits.length];

        System.arraycopy(digits, 0, digitsNew, 0, digits.length);

        for (int i = 0; i < precisionB; i++) {

            int carry = 0; //Keep a carry, initialize to 0

            for (int j = digitsNew.length - i - 1; j >= 0; j--) {
